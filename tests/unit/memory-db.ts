export type RoomRow = { id: string; name: string; created_at: number };
export type QuestionRow = { id: number; room_id: string; body: string; created_at: number };
...
export function createMemoryDb() {
  ...
  function exec(sql: string, values: unknown[]): ExecResult {
    ...
    if (q.startsWith("INSERT INTO rooms")) {
      rooms.push({ id: String(values[0]), name: String(values[1]), created_at: Number(values[2]) });
      return { first: null, results: [] };
    }
    if (q.startsWith("SELECT id, name FROM rooms WHERE id")) {
      const row = rooms.find((r) => r.id === String(values[0]));
      return { first: row ? { id: row.id, name: row.name } : null, results: row ? [{ id: row.id, name: row.name }] : [] };
    }
    ...
    if (q.startsWith("SELECT id FROM questions WHERE id")) {
      const row = questions.find((x) => x.id === Number(values[0]) && x.room_id === String(values[1]));
      return { first: row ? { id: row.id } : null, results: row ? [{ id: row.id }] : [] };
    }
    if (q.startsWith("SELECT id, body, created_at FROM questions")) {
      const roomId = String(values[0]);
      let rows = questions.filter((x) => x.room_id === roomId).sort((a, b) => b.id - a.id).slice(0, 200);
