...
勉強会やイベント用の 1 画面ボード。上部のフォームから匿名で質問を投稿すると、新しい順のカード一覧に載る。各カードから回答をぶら下げられ、回答は投稿先の質問にだけ表示される。ツールバーの「未回答のみ」を入れると、回答が 0 件の質問だけが残る。「表示中 N件」は今見えているカード数と一致する。投稿は D1 に残るので、別ブラウザで開いても同じ一覧が見える。ページ下部に使い方とよくある質問がある。
